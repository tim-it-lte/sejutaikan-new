                        ...
                        <div id="table" class="table-editable">
                            ...
                            <div class="alert alert-danger mt-3" role="alert">
                                <p style="font-style: italic;">
                                    jika status <a href="#">Sudah Verifikasi CS</a> maka silahkan verifikasi kembali permohonan anda!
                                </p>
                            </div>
                            @if($c->count()>= 1)
    ...
    <script>
        $(function() {
            $('#data_users_side').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('pemohon.datatable-all.permohonan') }}",
            });
        });
    </script>
    ...
    <script>
        function delData(id)
        {
            if(confirm("Data akan dihapus ?")){
                $.ajax({
                    url: "{{ url('/pemohon/permohonan/destroy') }}"+'/'+id,
                    type:'GET',
                    data: {},
